...
This section explains how to setup the Continous Dpeloyment (CD) of the defined configurations on Ansible controller when a event occurs(usally a merge event) in the git reposository where the definitions are kept.
...
1. Configure a Project and a job template with [webhook](https://docs.ansible.com/automation-controller/latest/html/userguide/webhooks.html#id2) property enabled in the Controller pointing to the playbook in charge of CD, you can find an example [here](ci-gitlab-webhook-trigger.yml). 
