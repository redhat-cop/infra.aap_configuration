...
|`eda_projects`|`see below`|yes|Data structure describing your projects, described below. Note that this role will only do anything if the `sync` suboption of this variable is set to true.||
...
|`eda_configuration_project_sync_async_retries`|`eda_configuration_async_retries`|no|This variable sets the number of retries to attempt for the role.|
|`eda_configuration_async_delay`|1|no|This sets the delay between retries for the role globally.|
|`eda_configuration_project_sync_async_delay`|`eda_configuration_async_delay`|no|This sets the delay between retries for the role.|
...
|`sync`|false|no|bool|Whether to sync the project. By default it will not sync unless this is set to true.|
|`wait`|true|no|str|Whether to wait for the sync to complete|
|`interval`|`eda_configuration_project_sync_async_delay`|no|str|The interval which the sync task will be checked for completion|
...
```yaml
---
- name: Sync project to EDA Controller
  hosts: localhost
  connection: local
  gather_facts: false
  vars:
