...
| `assign_galaxy_credentials_to_org`     | `true`        | no       | Boolean to indicate whether credentials should be assigned or not. It should be noted that credentials must exist before adding it. The dispatch role handles this internally via two passes. Set this to `false` at the playbook level to override all dispatch passes (useful on fresh platforms where credentials do not yet exist). |           |
| `assign_default_ee_to_org`             | `true`        | no       | Boolean to indicate whether default execution environment should be assigned or not. It should be noted that execution environment must exist before adding it. The dispatch role handles this internally via two passes. Set this to `false` at the playbook level to override all dispatch passes.                                   |           |
| `assign_notification_templates_to_org` | `true`        | no       | Boolean to indicate whether notification templates should be assigned or not. It should be noted that the templates must exist before adding them. The dispatch role handles this internally via two passes. Set this to `false` at the playbook level to override all dispatch passes.                                                 |           |
| `assign_instance_groups_to_org`        | `true`        | no       | Boolean to indicate whether an instance group should be assigned or not. It should be noted that the instance group must exist before adding it. Set this to `false` at the playbook level to override all dispatch passes.                                                                                                            |           |
