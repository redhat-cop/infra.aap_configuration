...
- allow for using hub_token for EE/collection management
- eda_rulebook_activations - Add per-object ``force_restart`` option to stop a running rulebook activation before applying changes and start it again afterward.
...
- Default aap_request_timeout to 10 seconds in global_vars and coerce empty or unset credential-injected values to that default in all roles so GitOps jobs using the Red Hat Ansible Automation Platform credential without request_timeout configured do not pass an invalid empty string to module request_timeout parameters.
...
- configure_aap.yml: `Added ignore_unknown_extensions: true` to include_vars task to prevent errors when non-YAML files (e.g., README.md) are present in the configs directory
...
- controller_job_templates, controller_workflow_job_templates, controller_schedules - Preserve explicitly empty association lists such as ``labels: []`` so they clear existing associations instead of being converted to omit (https://github.com/redhat-cop/infra.aap_configuration/issues/1430).
- controller_workflow_job_templates - Do not send an empty workflow-level limit when ``limit`` is unset or null. Exported workflow definitions commonly use ``limit: null``, which was rendered as an empty string and stored in ``char_prompts``, causing workflow node limits to be ignored at runtime.
